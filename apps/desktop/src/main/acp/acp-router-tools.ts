 ...
 *
 * @param toolName - The full tool name (e.g., 'vibecode-builtin:list_available_agents')
...
export async function executeACPRouterTool(
  ...
  try {
    ...
    switch (resolvedToolName) {
      case 'vibecode-builtin:list_available_agents':
        result = await handleListAvailableAgents(args as { capability?: string; skillName?: string });
        break;

      case 'vibecode-builtin:delegate_to_agent':
      ...
      case 'vibecode-builtin:check_agent_status':
